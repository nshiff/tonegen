...
void loop() {
  ...
  for (int thisPin =lowestPin; thisPin <= highestPin; thisPin++) { 
    ...
    int duration = 500;
    if(thisPin == 8){  //speaker plugged into pin 8
      for(int wavelengths=0; wavelengths<duration; wavelengths++){
        for(int i=0; i<120; i++){ //array is 120 elements long
          analogWrite(thisPin, waveformsTable[0][i]);
        }
      }
      delay(100);
      for(int wavelengths=0; wavelengths<duration; wavelengths++){
        for(int i=0; i<120; i++){ //array is 120 elements long
          analogWrite(thisPin, waveformsTable[1][i]);
        }
      }
      ...
      for(int wavelengths=0; wavelengths<duration; wavelengths++){
        for(int i=0; i<120; i++){ //array is 120 elements long
          analogWrite(thisPin, waveformsTable[2][i]);
        }
      }      
